...
import { axiomClient } from '../../axiom/init'
...
export const logger = new Logger({
  transports: [
    ...
    new ProxyTransport({ url: '/api/logger/axiom', autoFlush: true }),
  ],
  formatters: nextJsFormatters,
})

const useLoggerDefault = createUseLogger(logger)
const WebVitals = createWebVitalsComponent(logger)

const useLogger = () => {
  const log = useLoggerDefault()
  /**
  ...
  return (
    ...
    return log[level.toLowerCase() as 'info' | 'warn' | 'error' | 'debug'](
...
export { useLogger, useLoggerDefault, WebVitals }
